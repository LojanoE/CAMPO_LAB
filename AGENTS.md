...
All three modules can be exported to Excel: Presiómetro and Granulometría via `exportPresioExcel()` / `exportGranExcel()` (which read the live form), and PRA via `exportPraExcel(test)` (data-driven from a saved test object — `General` / `PesosParciales` / `Resultados` sheets — since v1.1.22; the PRA view's **Excel** button calls `exportPraExcelCurrent()`, which saves first). All three modules include a native **Exportar PDF** button that opens a print-formatted "Mina Mirador" field record sheet with a signature block; the user saves it as PDF from the browser print dialog. Each module also provides an inline signature pad so the operator can sign directly on screen; the signature is saved with the test and rendered in the PDF.

Each saved test in the home list also has **Excel** and **PDF** buttons (`exportTestFromHome(type, id, format)` in `buildTestItem`) so tests can be exported without opening them. The Excel path for presio/gran loads the test into the module form in silent mode — `loadPresioTest` / `loadPraTest` / `loadGranTest` accept `opts.noNav` to skip `showView()` — runs the DOM-based exporter, then restores the previously loaded test (or clears the form). The PDF path loads the test with navigation and calls `printFieldSheet('homeView')`; `printFieldSheet(returnTo)` accepts an optional return view so the user lands back on the home screen after the print dialog closes.

#### Numeric keypad (shared by PRA, Presiómetro and Granulometría) and PRA row ergonomics

Field crews enter 20–50 partial weights per group, so PRA has extra input ergonomics on top of the auto-adding rows above. Since v1.1.21 the on-screen keypad is shared by all three modules:
...
- `praKeypadMode` — `'1'`/`'0'`, whether the on-screen numeric keypad replaces the system keyboard (shared by PRA, Presiómetro and Granulometría). Defaults to on (`'1'`-equivalent) when unset; only an explicit `'0'` turns it off.
